...
def main():
    ...
    for z in range(1,11):
        ...
        train_label = torch.tensor(label_vector).to(torch.long).to(device)
        train = torch.tensor(feature).to(torch.float32).to(device)
        ...
        i = 0
        b = 0
        feature_vector_one_hot_test = np.zeros((640, 41, 4))
        feature_vector_chemical_test = np.zeros((640, 41, 4))
        for line in test_samples:
            if i % 2 != 0:
                label_vector_test.append(1)
                sequence = line.strip()
                feature_vector_one_hot_test[b] = AA_ONE_HOT(sequence)
                feature_vector_chemical_test[b] = chemical_properties(sequence)
                b = b + 1
            i = i + 1
        test_samples.close()
        ...
        feature_test = np.concatenate((feature_vector_chemical_test, feature_vector_one_hot_test), axis=2)

        test_label = torch.tensor(label_vector_test).to(torch.long).to(device)
        test = torch.tensor(feature_test).to(torch.float32).to(device)
        ...
        with torch.no_grad():
            ...
            predict_y_test = prediction_labels_end
            y_test = y_test_class
            TP = 0
            TN = 0
            FP = 0
            ...
            precision, recall, thresholds1 = precision_recall_curve(y_test_class, probs_end1, pos_label=1,
                                                                      sample_weight=None)
            a1 = average_precision_score(y_test_class, probs_end1, average='macro', pos_label=1, sample_weight=None)
